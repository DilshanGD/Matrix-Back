...
router.post("/staff-individual", checkSchema(individualStaffValidation), async (req, res) => {
    ...
    try {
        ...
        //     staffClass.map(async (cls) => {
        //         const batches = await Models.ClassBatch.findAll({ where: { title: cls.title } });
        //         return { title: cls.title, batches };
        //     })
        // );
        ...
        const cleanStaffClass = staffClass.map(({ title, type, location, day, startTime, endTime, batch_id, fee }) => ({
            title,
            type,
            location,
            day,
...
router.get("/timetable", async (req, res) => {
    try {
        const [staff, staffClass, subjects] = await Promise.all([
            Models.Staff.findAll(),
            Models.Class.findAll(),
            Models.Subject.findAll(),
        ]);

        // Create a mapping for staff full names
        const staffMap = staff.reduce((map, { username, full_name }) => {
            map[username] = full_name;
            return map;
        }, {});

        // Create a mapping for subject titles
        const subjectMap = subjects.reduce((map, { sub_id, title }) => {
            map[sub_id] = title;
            return map;
        }, {});

        // Group classes by subject
        const groupedClasses = staffClass.reduce((acc, cls) => {
            const subject = cls.sub_id; // Using sub_id to find the subject
            if (!acc[subject]) {
                acc[subject] = [];
            }
            const staffFullName = staffMap[cls.username] || 'Unknown Staff';
            acc[subject].push({
                title: cls.title,
                type: cls.type,
        ...
        const responseData = {
            class: Object.entries(groupedClasses).map(([subject, classes]) => ({
                subject: subjectMap[subject] || 'No Subject',
                classes
            }))
        };
