...
import { studentRegistrationValidation, studentLoginValidation, studentUpdateValidation, studentPWDValidation } from '../utils/studentDetailsValidation.mjs';
import { isStudentAuth } from '../utils/studentMiddleware.mjs';    // Authentication middleware
...
router.post("/student/student-login",checkSchema(studentLoginValidation), async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(
            {reg_no: findStudent.reg_no, full_name: findStudent.full_name, profile_pic: findStudent.profile_pic, type: "student" },
...
//     try {
//         const findStudent = await Models.Student.findOne({ where: { reg_no: req.user.reg_no }});  // Load relevent table
//         const findParent = await Models.Parent.findOne({ where: { reg_no: req.user.reg_no }});  // Load relevent table
//         return res.status(200).send(findStudent,findParent);
//     } catch (err) {
//         //console.log(err.message);
//         return res.status(400).send(err.message);
//     }
// })
router.post("/student/student-profile", isStudentAuth, async (req, res) => { 
    try {
      ...
      if (!student || !parent) {
        return res.status(404).json({ message: 'Student or parent data not found' });
      }
    ...
    try {
        if(data.email){
            ...
            const emailExist = await Models.Student.count({ 
                where: { 
                    email: data.email, 
                    reg_no: { [Op.ne]: req.user.reg_no }  // Exclude current student's reg_no
                } 
            });
            if (emailExist > 0) {
                return res.status(409).send("Already-Registered-email");
...
// Update student password api
router.patch("/student/new-password", isStudentAuth, checkSchema(studentPWDValidation), async (req, res) => {
    const result = validationResult(req);
...
router.post("/student/student-topbar", isStudentAuth, async (req, res) => {
    try {
...
// Classes page api for particular subject

// Class subscribe api

// Class unsubscribe api
